...
/*   Updated: 2019/07/13 23:01:31 by hhow-cho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_new_path(char *new_path, char **list)
{
	...
	while (list && list[i])
	{
		...
		if (list[i + 1] != 0  && ft_strlen(list[i + 1]) != 0 && new_path[ft_strlen(new_path) - 1] != '/')
			ft_strcat(new_path, "/");
		i++;
	}
...
char		*ft_path_trim(char *abs_path)
{
	...
	if (!(new_path = (char *)ft_memalloc(sizeof(char) \
		* (ft_strlen(abs_path) + 1))))
	{
		ft_list_free(&list);
		return (NULL);
	}
